...
При нажатии на кнопку "Новая игра" еды становится больше на поле.
Необходимо обнаружить эту ошибку и исправить ее.
-----------------------------------------------------------------

Причина бага находится в методе render объекта map (168 строка кода) -
а именно в строке this.usedCells.push(foodPoint).
...
По ходу выполнения кода, константе foodCell присваевается значение -
элемент html, который находится в объекте map.cells.
Указанному элементу добавляется класс food.
Однако, в массив usedCells добавляется не указанный элемент html,
а объект, переданный вторым аргументом функции.
Соответсвенно при итерации цикла указанному элементу не может быть присвоен
класс cell.

Соответсвенно, для корректной работы необходимо передать в
