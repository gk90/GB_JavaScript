...
решение:
1. Удалил проверку выхода за границы в game.CanMakeStep().
2. В объект snake добавил метод setMaxCoordinates(x, y).
3. В game.reset() добавил инструкцию this.snake.setMaxCoordinates(), данная функция принимаюет значения количества
рядов и колонок в игре.
4. Чтобы избежать бага, при котором невозможно съесть еду, сгенерированную на границе таблицы, при
пересечении змейкой границы с другого конца, изменил метод snake.getNextStepHeadPoint(), а не snake.makeStep().
*/
"use strict";
...
const snake = {
  ...
  makeStep() {
    ...
    // Вставляем значения следующий точки в начало массива.
...
/**
 * Объект счетчик. Ведет подсчет очков пользователя.
 * @property {int|null} count Количество очков,
 * @property {HTMLElement} countEl DOM-элемент для вставки количества очков
 */
const score = {
  count: null,
  countEl: null,
  ...
  /**
   * Инициализация объекта. Находит DOM-элемент.
   * Вызывает метод drop()
   */
  init() {
 ...
 * @property {score} score Объект счетчик очков.
 */
const game = {
  config,
  map,
